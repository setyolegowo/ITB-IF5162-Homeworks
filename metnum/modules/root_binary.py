...
class RootBinary(Common):
    def __init__(self, argv):
        if len(argv) < 3:
            raise Exception('Lack of argument to run root finding with binary method')
        self.function = argv[0]
        self.x_low = float(argv[1])
        self.x_high = float(argv[2])
        self.relative_error = float(argv[3]) if len(argv) > 3 else RootBinary.default_error()
        self._performance = {}
